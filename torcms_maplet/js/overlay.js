(function () {
    $(document).ready(function () {
        var baseMaps, ii, jj, lyrs, map, mycars, osm, overlayMaps, aa, mp_uid;
        var app_url = $("#app_ctrl").val(), app_arr = app_url.split("/")
        for (aa = 0; aa < app_arr.length;) {
            if (app_arr[aa].substring(0, 1) == 'm') {

                mp_uid = "mp" + app_arr[aa].substring(1, 5);

            } else {

                mp_uid = "qn" + app_arr[aa].substring(1, 5);

            }
            aa++
        }

        for (lyrs = new L.LayerGroup, mycars = new Array, jj = 0; jj < app_arr.length;)
