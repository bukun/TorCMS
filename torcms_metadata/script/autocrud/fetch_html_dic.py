...
from .base_crud import FILTER_COLUMNS, INPUT_ARR, XLSX_FILE_TAG,XLSX_FILE
...
if os.path.exists(XLSX_FILE_TAG):
    WORK_BOOK_TAG = load_workbook(filename=XLSX_FILE_TAG)
else:
    print('There must be at least one XLSX file.')
    sys.exit(0)
...
def gen_array_crud_tag():
    ...
    for work_sheet in WORK_BOOK_TAG:
