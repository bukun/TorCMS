...
$("#sub_reset").click(function () {
    ...
    if (currentZoom = 0, currentX = 0, currentY = 0, map_uid = "", $("#btn_updatemap").click(function () {
        ...
        $("#map").hasClass("mapdiv") || $("#map").css({
            ...
            return popup.setLatLng(e.latlng), popup.setContent("Coordinate position" + e.latlng.toString()), currentZoom = map.getZoom(), cmap_coor = e.latlng, link_str = "http://drr.ikcest.org/map/m" + map_uid + "?zoom=" + currentZoom + "&lat=" + cmap_coor.lat.toFixed(4) + "&lon=" + cmap_coor.lng.toFixed(4) + "&marker=1", "" !== geojsonid && (link_str = link_str + "&geojson=" + geojsonid), div_str = '{"i" : "' + map_uid + '",  "x" : ' + cmap_coor.lng.toFixed(3).toString() + ', "y": ' + cmap_coor.lat.toFixed(3).toString() + ', "v":' + currentZoom.toString() + ', "m": ' + vzoom_max + ', "n": ' + vzoom_min + ', "g": "' + geojsonid + '", "k": 1}', map_add_log(div_str,mapson), div_str = '&lt;div id="map" data-map = \'' + div_str + "'&gt;&lt;/div&gt;", $("#current_view_url").html(link_str), $("#mapref").html(div_str), $("#current_view_url").attr("href", link_str), popup.openOn(map)
        }, onZoomend = function () {
            var cmap_coor, div_str, link_str;
