...
class LabelsList(generics.ListCreateAPIView):
    queryset = LiteratureAuthor.objects.filter(sites__id=current_site.id)
...
def LabelDataList(request, pk):
    label_rec = get_object_or_404(LiteratureAuthor, pk=pk)
    ...
    context = {'data': data_recs, 'is_paginated': is_paginated,'label_name': label_rec.name,'Category':all_cat,'parent_template': parent_template}
