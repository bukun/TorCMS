...
class DataList(generics.ListCreateAPIView):
    queryset = LinearFeatures.objects.all()
...
def ajax_load_cities(request, region):
    if request.method == 'GET':
        ...
        reg_rec = LinearFeatures.objects.filter(region=region)
...
def map_view(request):
    region_recs = LinearFeatures.objects.values_list('region', flat=True).distinct()
    ...
    return render(request, 'linear_features/map_view.html', context)
...
def get_city(request, lat, lng):
    ...
    # 获取距离这个坐标最近的城市
    closest_city = LinearFeatures.objects.annotate(
...
def get_by_id(request, id):
    reg_rec = LinearFeatures.objects.filter(id=id)
    dict_list = [instance.to_dict() for instance in reg_rec]
    my_json = json.dumps(dict_list, ensure_ascii=False)
