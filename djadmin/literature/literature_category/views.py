...
class CategoryList(generics.ListCreateAPIView):
    queryset = LiteratureCatagory.objects.filter(sites__id=current_site.id)
...
class CategoryListView(ListView):
    ...
    context_object_name = 'literature_category'
    Category = LiteratureCatagory.objects.filter(sites__id=current_site.id)
...
def Categorylist(request):
    ...
    return render(request, 'literature_category/category_list.html', context)


def CategoryIndex(request, pk):
    all_cat = LiteratureCatagory.objects.filter(sites__id=current_site.id)
    context = {'Category': all_cat,'parent_template': parent_template}
    return render(request, 'literature_category/category_index.html', context)


def CategoryDataList(request, pk):
    category_rec = get_object_or_404(LiteratureCatagory, pk=pk)
    all_cat = LiteratureCatagory.objects.filter(sites__id=current_site.id)
    data_recs = category_rec.literature_data.filter(sites__id=current_site.id)
    ...
    return render(request, 'literature_category/data_list.html', context)


def index(request):
    category_rec = LiteratureCatagory.objects.filter(sites__id=current_site.id).order_by('order')

    cat_rec = []
    for cat in category_rec:
        data = cat.literature_data.filter(sites__id=current_site.id)[:4]
    ...
    return render(request, 'literature_category/index.html', context)
